...
function CategoryBadge({ category }: { category: string | null }) {
  ...
  const c = CATEGORY_COLORS[category] ?? { bg: "bg-gray-100", text: "text-gray-500" };
  return (
...
function MemberAvatar({ member }: { member?: string | null }) {
  if (!member) return <span className="text-gray-300">—</span>;
  return (
...
function AmountCell({ amount, currency }: { amount: number; currency: string }) {
  return (
    <span className="font-semibold tabular-nums text-emerald-700">
      {currency !== "USD" && (
        <span className="mr-0.5 text-xs font-normal text-gray-400">{currency}</span>
      )}
      ${amount.toFixed(2)}
    </span>
  );
...
function ActionsMenu({ id }: { id: string }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="rounded-lg p-1.5 text-gray-300 opacity-0 transition group-hover:opacity-100 hover:bg-gray-100 hover:text-gray-600 focus:opacity-100"
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-20 mt-1 w-40 rounded-xl border border-gray-100 bg-white py-1 shadow-lg ring-1 ring-black/5">
            <button
              onClick={handleDelete}
              disabled={pending}
              className="flex w-full items-center gap-2.5 px-3 py-2 text-sm text-red-600 transition hover:bg-red-50 disabled:opacity-50"
            >
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-white py-16 text-center">
      ...
      <p className="mt-1 text-xs text-gray-400">Add your first expense using the form above.</p>
    </div>
  );
}
...
const TH = "px-4 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-gray-400";
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between px-0.5">
        ...
        <p className="text-sm font-semibold tabular-nums text-gray-900">
          Total: <span className="text-emerald-700">${total.toFixed(2)}</span>
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {expenses.map((e) => (
              <tr key={e.id} className="group transition-colors hover:bg-gray-50">
                {/* Description */}
                <td className="px-4 py-2.5">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-gray-900">{e.title}</span>
                    {e.is_recurring && (
                      <span className="rounded-full bg-indigo-50 px-2 py-px text-xs font-medium text-indigo-600">
                        recurring
                      </span>
                    )}
                  </div>
                  {e.notes && (
                    <p className="mt-0.5 max-w-xs truncate text-xs text-gray-400">{e.notes}</p>
                  )}
                </td>
                ...
                {/* Date */}
                <td className="px-4 py-2.5 text-right">
                  <span className="tabular-nums text-xs text-gray-400">{e.date}</span>
                </td>
