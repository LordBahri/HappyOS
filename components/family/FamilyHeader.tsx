...
export default function FamilyHeader({
  ...
  // Close edit mode after a successful save (pending flips false with no error)
  useEffect(() => {
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-2xl font-bold leading-tight text-gray-900">{family.name}</h1>
          ...
          {isAdmin && (
            <button
              ...
              className="flex shrink-0 items-center gap-1.5 rounded-xl border border-gray-200 px-3 py-2 text-xs font-medium text-gray-600 transition hover:bg-gray-50 hover:text-gray-900"
            >
  ...
  return (
    <div className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm ring-1 ring-indigo-100">
      ...
      <form action={formAction} className="space-y-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-600">Family Name</label>
            <input
              name="name"
